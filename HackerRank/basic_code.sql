...
--Q6: 
/*
A median is defined as a number separating the higher half of a data set from the lower half. Query the median of 
...
the respective hacker_id and name of hackers who achieved full scores for more than one challenge. 
...
buy each non-evil wand of high power and age. Write a query to print the id, age, coins_needed, and power of the wands 
that Ron's interested in, sorted in order of descending power. If more than one wand has same power, 
...
the total number of challenges created by each student. Sort your results by the total number of challenges in descending order. 
...
FROM Friends as F
     ...
     INNER JOIN Packages as P1 ON P1.ID = F.Friend_ID
